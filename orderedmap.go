...
func (o *OrderedMap) IsZero() bool {
	...
	return len(o.keys) == 0 && len(o.values) == 0
}
...
func (o *OrderedMap) DeepCopyInto(out *OrderedMap) {
	*out = *o
	out.keys = make([]string, len(o.keys))
	copy(out.keys, o.keys)
	out.values = DeepCopyJSONValue(o.values).(map[string]interface{})
	return
}
...
func DeepCopyJSON(x *OrderedMap) *OrderedMap {
	return DeepCopyJSONValue(x).(*OrderedMap)
}
...
func DeepCopyJSONValue(x interface{}) interface{} {
	...
	case *OrderedMap:
		...
		for _, k := range x.keys {
			v, _ := x.Get(k)
			clone.Set(k, DeepCopyJSONValue(v))
		}
		return clone
	case []interface{}:
		if x == nil {
